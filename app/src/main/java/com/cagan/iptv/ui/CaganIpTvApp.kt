...
private fun ChannelScreen(
    ...
    var selectedGroup by remember(channels) { mutableStateOf("Tüm Kanallar") }
    ...
    Row(Modifier.fillMaxSize().background(Bg)) {
        Column(
            ...
            Text("Cagan", fontSize = 30.sp, fontWeight = FontWeight.Bold, color = Color.White)
            Text("IP TV", fontSize = 20.sp, color = Accent, fontWeight = FontWeight.Bold)
            ...
            Button(
                ...
                onClick = { selectedGroup = "Tüm Kanallar" }
            ) { Text("Tüm Kanallar") }
...
private fun parseStreamRequest(rawUrl: String): StreamRequest {
    ...
    if (headers.keys.none { it.equals("User-Agent", ignoreCase = true) }) {
        headers["User-Agent"] = "Cagan-IP-TV/0.4 (Android)"
    }
